	  ...
	  /* --- Some miscellaneous variables to format the string output --- */
		...
		switch(display_freq)
			...
			if(1461)
				freq_text = "Romero"
			if(1363)
				freq_text = "#hazteam_echo"
			if(1365)
				freq_text = "Marine Command"
			if(1367)
				freq_text = "Bravo Squad"
			if(1369)
				freq_text = "Alpha Squad"
			if(1371)
				freq_text = "Charlie Squad"
			if(1379)
				freq_text = "Military Police"
			if(1381)
				freq_text = "Romero Engineer"
		...
		else if (display_freq == MSUL_FREQ)
			part_a = "<span class='centradio'><span class='name'>"

		else if (display_freq == MHAZ_FREQ)
			part_a = "<span class='syndradio'><span class='name'>"

		else if (display_freq == MCOM_FREQ)
			part_a = "<span class='comradio'><span class='name'>"

		else if (display_freq == MBRA_FREQ)
			part_a = "<span class='braradio'><span class='name'>"

		else if (display_freq == MALP_FREQ)
			part_a = "<span class='alpradio'><span class='name'>"

		else if (display_freq == MCHA_FREQ)
			part_a = "<span class='charadio'><span class='name'>"

		else if (display_freq == MGRA_FREQ)
			part_a = "<span class='graradio'><span class='name'>"

		else if (display_freq == MMPO_FREQ)
			part_a = "<span class='mporadio'><span class='name'>"

		else if (display_freq == MDEL_FREQ)
			part_a = "<span class='delradio'><span class='name'>"
		...
		if(istype(blackbox))
			switch(display_freq)
				...
				if(1461)
					blackbox.msg_msul += blackbox_msg
				if(1363)
					blackbox.msg_mhaz += blackbox_msg
				if(1365)
					blackbox.msg_mcom += blackbox_msg
				if(1367)
					blackbox.msg_mbra += blackbox_msg
				if(1369)
					blackbox.msg_malp += blackbox_msg
				if(1371)
					blackbox.msg_mcha += blackbox_msg
				if(1379)
					blackbox.msg_mmpo += blackbox_msg
				if(1381)
					blackbox.msg_mdel += blackbox_msg
